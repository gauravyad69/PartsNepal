package np.com.parts.Presentation.ui.statusbar
...
/**
 * Extension function on an [Activity] to insets a top padding using the [StatusBarColors.getDefaultStatusBarHeight] on the [view].
 *
    ...
    view.apply {
        layoutParams?.let {
            if (it.height > 0 && paddingTop == 0) {
                it.height += StatusBarColors.getDefaultStatusBarHeight(this@setPaddingTop)
                setPadding(
                    paddingLeft, paddingTop + StatusBarColors.getDefaultStatusBarHeight(this@setPaddingTop),
